...
function Estimator({ onEstimate }: { onEstimate: (e: Estimate) => void }) {
  ...
  const debouncedFetch = React.useMemo(() =>
    debounce(async (q: string, which: "from" | "to") => {
      if (!q || q.length < 2) { (which === "from" ? setFromSug : setToSug)([]); return; }
  ...
  React.useEffect(() => { debouncedFetch(from, "from"); setFromPick(null); }, [from, debouncedFetch]);
  React.useEffect(() => { debouncedFetch(to, "to"); setToPick(null); }, [to, debouncedFetch]);

  async function estimate() {
    setError(null);
    if (!fromPick || !toPick) { setError("Choisis les adresses dans la liste."); return; }
    setLoading(true);
    const r = await fetch(`/api/route?from=${fromPick.lon},${fromPick.lat}&to=${toPick.lon},${toPick.lat}`);
    const data: { km?: number; minutes?: number } = await r.json();
    setLoading(false);
    if (!r.ok || !data.km || !data.minutes) { setError("Itinéraire introuvable."); return; }
    const km = data.km, minutes = data.minutes;
    const airport = ["aéroport","marseille provence","mrs"].some(w =>
      [fromPick.name,toPick.name].some(n=>n.toLowerCase().includes(w)));
    let price = Math.max(12, 5 + 2.2 * km);
    if (airport) price = 39;
    onEstimate({ price: Math.round(price) + "€", km, minutes });
  }

  const Suggest = ({items,onPick}:{items:Place[];onPick:(it:Place)=>void}) =>
    !items.length ? null : (
      <div className="border rounded-md bg-white shadow-sm max-h-48 overflow-auto">
        {items.map(it=>(
          <button key={it.id} className="block w-full text-left px-3 py-2 hover:bg-slate-50" onClick={()=>onPick(it)}>
            {it.name}
          </button>
        ))}
      </div>
  ...
  return (
    <Card className="shadow-xl">
      ...
      <CardContent className="grid gap-3">
        <div>
          <Input value={from} onChange={e=>setFrom(e.target.value)} placeholder="Départ (ex: Vieux-Port)" />
          <Suggest items={fromSug} onPick={(it)=>{ setFrom(it.name); setFromPick(it); setFromSug([]); }} />
        </div>
        <div>
          <Input value={to} onChange={e=>setTo(e.target.value)} placeholder="Arrivée (ex: Aéroport MRS)" />
          <Suggest items={toSug} onPick={(it)=>{ setTo(it.name); setToPick(it); setToSug([]); }} />
        </div>
